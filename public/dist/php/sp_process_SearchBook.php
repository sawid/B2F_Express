...
if ($actions == '1911') {
    ...
    if ($num != 0) {
        ...
        if (isset($data) && $data[0] != '') {
            ...
            $sql_final = " SELECT *  FROM book_data  a left join data_book_call_classification_100 b on a.book_call_classification_100 = b.book_call_classification_100 where  a.book_id IN (".$ref_id_final_filter.") ORDER BY a.book_id ".$sort." ";
            $stmt_final = sqlsrv_query($conn, $sql_final);
            if ($stmt_final === false) {
                die(print_r(sqlsrv_errors(), true));
                // $data .= '<input type="hidden" name="count_data" id="count_data" class="form-control" value="'.count($ref_id_final).'">';
